...
(function () {
  ...
  // 读取 WAV 文件头中的真实采样率，用于「保持原样」时不被重采样
  ...
  // 在尽量贴近目标采样率的上下文中解码，避免 decodeAudioData 强制重采样到设备采样率；
  // 若浏览器把上下文就近吸附到其它采样率，再用 resample 纠正回 preferredRate。
  function decodeAudioBuffer(buf, preferredRate) {
    return new Promise(function (resolve, reject) {
      ...
      catch (e) { try { ctx = new Ctx(); } catch (e2) { reject(new Error('无法创建音频上下文')); return; } }
  ...
  // 分块调用 lamejs，避免长时间阻塞主线程
  function encodeMp3(audioBuffer, kbps, onProg) {
    return new Promise(function (resolve, reject) {
      try {
        if (typeof lamejs === 'undefined') throw new Error('MP3 编码组件（lamejs）未加载');
  ...
  function resetAll(clearFile) {
    ...
    if (clearFile) {
      ...
      input.value = '';
      fileInfo.innerHTML = '';
      summary.textContent = '还没有加载音频';
      emptyText.textContent = '上传音频后设置参数，点击「开始转换」生成结果';
      empty.hidden = false;
    }
  ...
  function loadFile(file) {
    ...
    if (!okType) { FT.toast('请选择音频文件（MP3 / WAV / OGG / M4A 等）', 'err'); return; }
    if (file.size > MAX_SIZE) { FT.toast('音频超过 200MB，请压缩或裁剪后再试', 'err'); return; }
  ...
  function decodeForNative() {
    var targetRate = Number(nrate.value) || 0; // 0 = 保持原样
    return FT.readArrayBuffer(state.file).then(function (buf) {
      var parsed = parseWavSampleRate(buf);
      // 解码时尽量贴近最终采样率：指定了目标就用目标，否则用 WAV 的真实采样率
  ...
  function runNative() {
    ...
    var targetRate = Number(nrate.value) || 0;     // 0=保持
    ...
    decodeForNative().then(function (audioBuffer) {
      ...
      FT.setProgress(progress, 0, '正在编码 MP3…');
      return encodeMp3(audioBuffer, kbps, function (r) {
        FT.setProgress(progress, r, '编码 MP3 ' + Math.round(r * 100) + '%');
      }).then(function (blob) {
  ...
  function showResult(blob, ext, meta) {
    ...
    var rows = '';
    rows += stat('输出格式', ext.toUpperCase());
    if (meta && meta.dur) rows += stat('时长', fmtDuration(meta.dur));
    if (meta && meta.rate) rows += stat('采样率', meta.rate + ' Hz');
    if (meta && meta.ch) rows += stat('声道', meta.ch >= 2 ? '立体声' : '单声道');
    rows += stat('原始体积', FT.formatBytes(origSize));
    var cls = blob.size <= origSize ? 'down' : 'up';
    rows += stat('输出体积', FT.formatBytes(blob.size) + '（' + FT.pct(blob.size, origSize) + '）', cls);
