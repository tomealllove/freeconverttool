/* 证件照生成：上传 → 取景裁剪 → 换背景（纯色容差 / AI 分割）→ 预览下载 / 相纸排版 */
(function () {
  'use strict';
  ...
  function updateSizeInfo() {
    var t = targetSize();
    if (!t) {
      sizeInfo.textContent = '自定义尺寸格式不正确，请填写如 800×1000（50 ~ ' + MAX_SIDE + ' px）';
      return;
    }
    ...
    var label = preset.selectedOptions && preset.selectedOptions[0]
      ? preset.selectedOptions[0].textContent.split('（')[0] : '自定义';
    sizeInfo.textContent = label + ' · ' + t.w + '×' + t.h + ' px ≈ ' + mw + '×' + mh +
      ' mm（按 ' + DPI + ' DPI 换算，1 英寸 = ' + DPI + 'px）';
  }
  ...
  function draw() {
    ...
    if (!state.img) {
      ...
      ctx.fillText('请先上传一张人像照片', state.cssW / 2, state.cssH / 2);
      return;
    }
  ...
  stage.addEventListener('pointerdown', function (e) {
    if (!state.img) return;
    try { stage.setPointerCapture(e.pointerId); } catch (err) { /* 某些浏览器可能不支持 */ }
  ...
  function pickMask(res, w, h) {
    ...
    if (cm) {
      ...
      if (arr) {
        ...
        fg = (cnt && sum / cnt > 0.5) ? 1 : 0;   // 用画面中心判断哪一类是人物
      }
    }
  ...
  // AI 人像分割：动态 import MediaPipe，失败由调用方回退到纯色容差
    ...
    return Promise.resolve()
      ...
      .then(function (seg) {
        try {
          ...
          for (var i = 0; i < a.length; i++) a[i] = (a[i] - 0.25) / 0.35;   // 软化阈值
          return a;
        } finally {
          try { seg.close(); } catch (e) { /* 忽略释放异常 */ }
        }
      });
  }
  ...
  function generate() {
    ...
    if (!state.img) { FT.toast('请先上传一张人像照片', 'err'); return; }
    var t = targetSize();
    if (!t) {
      FT.toast(customWrap.hidden ? '尺寸预设无效' : '自定义尺寸格式不正确，请填写如 800×1000', 'err');
      return;
    }
    ...
    FT.setProgress(progress, 0.2, '正在裁剪取景…');
    ...
    FT.nextFrame().then(function () {
      ...
      if (matte === 'ai') {
        ...
        return aiAlpha(canvas).then(function (alpha) {
          ...
          return { canvas: canvas, note: 'AI 人像分割' };
        }).catch(function (err) {
          console.warn('AI 抠图失败，回退纯色容差：', err);
          FT.alert(tips, 'warn', '<b>AI 抠图不可用，已自动回退</b><p>' +
            FT.esc((err && err.message) || '模型加载失败') + '，本次改用「纯色容差」方案，可调整容差滑块改善效果。</p>');
          colorKey(canvas, bgHex, Number(tol.value), keepAlpha);
          return { canvas: canvas, note: '纯色容差（AI 回退）' };
        });
      }
      ...
      FT.setProgress(progress, 0.8, '正在编码输出…');
      return FT.canvasToBlob(canvas, fmt, 0.92).then(function (blob) {
        ...
        FT.setProgress(progress, 0.9, '正在排版相纸…');
      ...
      statsBox.innerHTML = [
        { k: '输出尺寸', v: t.w + '×' + t.h + ' px' },
        { k: '对应毫米', v: mw + '×' + mh + ' mm' },
        { k: '文件体积', v: FT.formatBytes(res.blob.size) },
        { k: '处理方式', v: res.note }
      ].map(function (s) {
      ...
      summary.textContent = '已生成 · ' + t.w + '×' + t.h + ' px · ' + res.note;
      ...
      FT.alert(tips, 'err', '<b>生成失败</b><p>' + FT.esc((err && err.message) || '图片处理异常') + '</p>');
    }).then(function () {
      state.busy = false;
  ...
  function loadFile(files) {
    ...
    if (!/^image\//.test(file.type) && ['png', 'jpg', 'jpeg', 'webp'].indexOf(ext) < 0) {
      FT.toast('请选择 JPG / PNG / WebP 图片', 'err');
      return;
    }
    if (file.size > MAX_SIZE) {
      FT.toast('图片超过 20MB，请压缩后再试', 'err');
      return;
    }
    FT.busyProgress(progress, '正在读取图片…');
    ...
    FT.loadImage(url).then(function (img) {
      ...
      empty.hidden = false;
      summary.textContent = '已载入 ' + file.name + '，请调整取景后生成';
      resizeCanvas();
      fitView();
      FT.resetProgress(progress);
      FT.toast('图片已载入', 'ok');
      ...
      FT.resetProgress(progress);
      FT.toast('图片读取失败：' + ((err && err.message) || err), 'err');
    });
  }
  ...
  downloadBtn.addEventListener('click', function () {
    if (!state.blob) { FT.toast('还没有可下载的图片', 'err'); return; }
    FT.saveBlob(state.blob, state.name);
  });
  paperBtn.addEventListener('click', function () {
    if (!state.paperBlob) { FT.toast('请先勾选「排版到 5 寸相纸」并重新生成', 'err'); return; }
