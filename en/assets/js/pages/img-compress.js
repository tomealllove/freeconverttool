/* Image Compressor: upload images → quality mode / target size mode → Canvas re-encode → compare preview / ZIP download */
(function () {
  'use strict';
  ...
  function cardHTML(item) {
    var tag = item.status === 'done' ? '<span class="badge ok">Done</span>'
      ...
      : item.status === 'working' ? '<span class="badge">Compressing…</span>'
      : '<span class="badge">Pending</span>';

    var out = item.status === 'done'
      ? '<img src="' + FT.esc(item.url) + '" alt="Compressed" data-zoom>'
      : '<div class="ph">' + (item.status === 'error' ? 'Compression failed' : 'Not compressed yet') + '</div>';

    var origCap = '<b>' + (FT.extName(item.name).toUpperCase() || 'ORIGINAL') + '</b> · ' +
      (item.w ? item.w + '×' + item.h : 'Loading…') + ' · ' + FT.formatBytes(item.file.size);
    var outCap = item.status === 'done'
      ? '<b>' + (MIME_EXT[item.mime] || '').toUpperCase() + '</b> · ' + item.ow + '×' + item.oh +
        ' · ' + FT.formatBytes(item.blob.size)
      : (item.status === 'error' ? FT.esc(item.error || 'Compression failed') : 'Waiting for compression');

    var statHtml = item.status === 'done'
      ? '<div class="stat-row">' +
          stat('Before', FT.formatBytes(item.file.size)) +
          stat('After', FT.formatBytes(item.blob.size)) +
          stat('Size change', FT.pct(item.blob.size, item.file.size),
            item.blob.size <= item.file.size ? 'down' : 'up') +
          stat('Output size', item.ow + '×' + item.oh) +
        '</div>'
      : '';
    ...
    return '' +
      ...
      '<div class="compare">' +
        '<figure><div class="ph checker"><img src="' + FT.esc(item.srcUrl) + '" alt="Original" data-zoom></div>' +
          '<figcaption>Before · ' + origCap + '</figcaption></figure>' +
        '<figure><div class="ph checker">' + out + '</div><figcaption>After · ' + outCap + '</figcaption></figure>' +
      '</div>' +
      statHtml +
      '<div class="item-actions">' +
        '<button class="btn btn-sm btn-primary" data-act="compress">Compress</button>' +
  ...
  function updateBar() {
    ...
    summary.textContent = !has ? 'No images added yet'
      : items.length + ' images · ' + done.length + ' compressed · original total ' +
  ...
  function renderStats() {
    ...
    stats.hidden = false;
    stats.innerHTML =
      stat('Original total', FT.formatBytes(before)) +
      stat('After', FT.formatBytes(after)) +
      stat('Size change', FT.pct(after, before), d >= 0 ? 'down' : 'up') +
      stat('Compressed', done.length + ' / ' + items.length);
  }
  ...
  function pickMime(item) {
    ...
    // Keep original format: Canvas can only encode png / jpeg / webp, everything else falls back to png
    var t = item.type;
  ...
  // Binary search towards the target size: keep the highest-quality result that still fits
  ...
  function compress(item, silent) {
    ...
    return FT.loadImage(item.srcUrl).then(function (img) {
      ...
      if (item.url) URL.revokeObjectURL(item.url);
      if (!res.blob) throw new Error('Image encoding failed');
      item.blob = res.blob;
      ...
      if (!silent) FT.toast('Failed to compress "' + item.name + '": ' + item.error, 'err');
    }).then(function () {
      refresh(item);
      updateBar();
  ...
  compressAll.addEventListener('click', function () {
    ...
    if (mode.value === 'target' && !(Number(targetKb.value) > 0)) {
      FT.toast('Please enter a target size (KB)', 'err');
      return;
    }
    var total = items.length, ok = 0, i = 0, missed = 0;
    FT.setBusy(compressAll, true, 'Compressing…');
    function step() {
      if (i >= total) {
        FT.setBusy(compressAll, false);
        FT.setProgress(progress, 1, 'Done ' + ok + '/' + total);
        FT.toast('Compression complete, ' + ok + ' succeeded' + (ok < total ? ', ' + (total - ok) + ' failed' : ''), ok ? 'ok' : 'err');
        if (missed) FT.toast(missed + ' image(s) could not reach the target size; the closest result was kept', 'warn');
        setTimeout(function () { FT.resetProgress(progress); }, 2200);
        return;
      }
      FT.setProgress(progress, i / total, 'Compressing ' + (i + 1) + '/' + total);
      i++;
  ...
  mode.addEventListener('change', function () {
    ...
    modeTip.textContent = isTarget
      ? 'Target size mode: binary-search the target size automatically, up to 6 iterations; quality is chosen by the algorithm.'
      : 'Quality mode: re-encode at a fixed quality — fast and predictable.';
  });
  ...
  if (!FT.supportsMime('image/webp')) {
    FT.alert(tips, 'warn', '<b>Your browser does not support WebP encoding</b>' +
      '<p>When you pick WebP or keep a WebP original, JPG output is used instead.</p>');
  }
  renderAll();
})();
